# 记录所有名片字典
card_list = []

def show_menu():

    """显示菜单"""
    print("*" * 40)
    print("欢迎使用【名片管理系统】V1.0")
    print("")

    print("1. 新建名片")
    print("2. 显示全部")
    print("3. 查询名片")
    print("")
    print("0. 退出系统")
    print("*" * 40)

def new_card():

    """新增名片"""
    print("-" * 50)
    print("新增名片")

    # 提示用户输入名片的详细信息
    name_str = input("请输入姓名：")
    phone_str = input("请输入phone：")
    qq_str = input("请输入qq：")
    email_str = input("请输入email：")

    # 使用一个用户输入的信息建立一个名片字典
    card_dict = {"name": name_str,
                 "phone": phone_str,
                 "qq": qq_str,
                 "email": email_str}
    # 将名片字典添加到列表里
    card_list.append(card_dict)
    print(card_list)
    # 提示用户添加成功
    print("添加 %s 成功" % name_str)

def show_all():

    """显示名片"""
    print("-" * 50)
    print("显示名片")

    # 如果名片中没有数据，应该提示用户并返回
    if len(card_list) == 0:
        print("没有数据，请使用新增功能添加数据")

        # return 可以返回一个函数的执行结果
        # 下方的代码不会被执行
        # 如果 return 后面没有任何内容，表示会返回到调用函数的位置
        # 并且不返回任何的结果
        return
    # 打印表头
    for name in ["姓名","电话","qq","邮箱"]:
        print(name,end="\t\t")

    print("")

    # 打印分隔线
    print("=" * 50)
    # 遍历列表中字典的信息
    for card_dict in card_list:
        print("%s\t\t%s\t\t%s\t\t%s" % (card_dict["name"],
                                        card_dict["phone"],
                                        card_dict["qq"],
                                        card_dict["email"]))
    print("=" * 50)

def search_card():

    """搜索名片"""
    print("-" * 50)
    print("搜索名片")

    # 提示用户输入要搜素的名字
    find_name = input("请输入要查找的名字：")

    # 遍历名片列表，如果找到就输出，如果没有就提示用户无法找到
    for card_dict in card_list:

        if card_dict["name"] == find_name:

            print("姓名\t\t电话\t\tqq\t\temail")
            print("-" * 50)
            print("%s\t\t%s\t\t%s\t\t%s" % (card_dict["name"],
                                            card_dict["phone"],
                                            card_dict["qq"],
                                            card_dict["email"]))

            # TODO 针对找到的名片记录执行修改和删除的操作
            deal_card(card_dict)

            break
        else:
            print("无法找到 %s ！" % find_name)

def deal_card(find_dict):
    """
处理查找到的名片
    :param find_dict: 查找到的名片
    """
    print(find_dict)

    action_srt = input("选择要执行的操作  【1】.修改"
                       "  【2】.删除"
                       "  【0】.返回上级菜单")

    if action_srt == "1":

        # 替换每个键值对中的数据
        find_dict["name"] = input_card_info(find_dict["name"],"姓名【回车不修改】：")
        find_dict["phone"] = input_card_info(find_dict["phone"],"电话【回车不修改】：")
        find_dict["qq"] = input_card_info(find_dict["qq"],"QQ【回车不修改】：")
        find_dict["email"] = input_card_info(find_dict["email"],"邮箱【回车不修改】")

        print("修改名片成功")

    elif action_srt == "2":

        card_list.remove(find_dict)

        print("删除名片成功")

def input_card_info(dict_value,tip_message):
    """输入要修改的名片

    :param dict_value:原字典中的值
    :param tip_message:提示用户输入文字
    :return:
    """
    # 1.提示用户输入内容
    result_str = input(tip_message)
    # 2.判断用户的输入，如果输入了内容，直接返回结果
    if len(result_str) > 0:

        return result_str
    # 3.如果没有输入内容，直接返回‘原字典中的值’
    else:
        return dict_value